...
Form D, already-local, passed through unchanged. An entry already written as a
relative !include (for example, because it was hand-authored to demonstrate
local use rather than generated by this tool) is emitted byte for byte, in
either the single-line shape or the file:/vars: mapping shape:
...
The passthrough is not a hole in the tool's strictness. The strictness exists
because a github:// reference silently points somewhere other than the
working tree; a relative include already points at the working tree, which is
the transform's goal. The path must still start with ../: examples/ and
.test-build/ are siblings one level below the repo root, so a ../-relative
include resolves identically from both, and that identity is the entire
justification for the passthrough. A path without ../ resolves to neither, so
it is rejected, not passed through.
...
ALREADY_LOCAL_INCLUDE_RE = re.compile(
    r"^(\s+)([A-Za-z0-9_-]+):\s+!include\s+(\S+)$"
)
ALREADY_LOCAL_MAPPING_RE = re.compile(r"^(\s+)([A-Za-z0-9_-]+):\s+!include\s*$")
ALREADY_LOCAL_FILE_RE = re.compile(r"^\s*file:\s*(\S+)$")
...
def transform_package(lines, i, path):
    ...
    # Form D, already-local single-line passthrough.
    match = ALREADY_LOCAL_INCLUDE_RE.match(stripped)
    if match:
        _pad, key, include_path = match.groups()
        if not include_path.startswith("../"):
            raise LocalizeError(
                path,
                i + 1,
                stripped,
                "already-local include %r for package %r does not start "
                "with ../; examples/ and .test-build/ only resolve a "
                "relative include identically when it is ../-relative"
                % (include_path, key),
            )
        ref_path = include_path[len("../"):]
        return [raw], 1, ref_path

    # Form D, already-local mapping passthrough (file: + vars:).
    match = ALREADY_LOCAL_MAPPING_RE.match(stripped)
    if match:
        _pad, key = match.groups()
        key_indent = indent_of(stripped)
        emitted = [raw]
        j = i + 1
        ref_path = None
        while j < len(lines):
            ...
            if indent_of(sub) <= key_indent:
                break
            emitted.append(sub_raw)
            file_match = ALREADY_LOCAL_FILE_RE.match(sub)
            if file_match and ref_path is None:
                ...
                if not file_path.startswith("../"):
                    raise LocalizeError(
                        ...
                        j + 1,
                        sub,
                        "already-local include %r for package %r does not "
                        "start with ../; examples/ and .test-build/ only "
                        "resolve a relative include identically when it is "
                        "../-relative" % (file_path, key),
                    )
                ref_path = file_path[len("../"):]
            j += 1
        if ref_path is None:
            raise LocalizeError(
                path,
                i + 1,
                stripped,
                "already-local package %r has no file:" % key,
            )
        consumed = j - i
...
packages:
  ...
  passthrough: !include ../hardware-rev-3.2.yaml
  passthroughvars: !include
    file: ../modules/temperature_linear.yaml
    vars:
      # a comment inside passthrough vars
      friendly_name: "Fancontroller"  # trailing comment
...
# The exact sorted list of repo-relative paths SELF_TEST_GOOD_EXPECTED must
# yield. self_test() asserts refs against this so a form that forgets to
# return its ref_path goes red immediately instead of shipping a green lie
# (see the D-06 assumption-delta decision in 20-01-PLAN.md).
SELF_TEST_GOOD_REFS = [
    "hardware-rev-3.1.yaml",
    "hardware-rev-3.2.yaml",
    "modules/rpm_status_leds.yaml",
    "modules/temperature_linear.yaml",
    "modules/usr_buttons.yaml",
]
...
SELF_TEST_BAD = [
    ...
    (
        "already-local include without ../ prefix",
        "packages:\n  hardware: !include hardware-rev-3.1.yaml\n",
    ),
]
...
def self_test():
    ...
    elif refs != SELF_TEST_GOOD_REFS:
        failures.append(
            "positive fixture refs mismatch\n--- got ---\n%s\n--- want ---\n%s"
            % (refs, SELF_TEST_GOOD_REFS)
        )
    else:
        print("ok: positive fixture, refs=%s" % refs)

    # Idempotency: localize() applied to its own output is a fixed point. This
    # also exercises Form D against every shape the other three forms produce,
    # since SELF_TEST_GOOD_EXPECTED is exactly that output.
    got2, refs2 = localize(SELF_TEST_GOOD_EXPECTED, "self-test-idempotency")
    if got2 != SELF_TEST_GOOD_EXPECTED:
        failures.append(
            "idempotency mismatch: localize(output) != output\n"
            "--- got ---\n%s\n--- want ---\n%s" % (got2, SELF_TEST_GOOD_EXPECTED)
        )
    elif refs2 != SELF_TEST_GOOD_REFS:
        failures.append(
            "idempotency refs mismatch\n--- got ---\n%s\n--- want ---\n%s"
            % (refs2, SELF_TEST_GOOD_REFS)
        )
    else:
        print("ok: idempotency, localize(output) is a fixed point, refs=%s" % refs2)
