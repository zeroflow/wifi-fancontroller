...
| `grp_${ow_prefix}_onewire` | DS2484 1-Wire | 330 | Sensors |
| `grp_usr_buttons` | USR Buttons | 400 | Controls |

`grp_${ow_prefix}_onewire`'s ID derives from that module's own `ow_prefix` substitution, so a second DS2484 instance gets its own group automatically instead of colliding with the first.
